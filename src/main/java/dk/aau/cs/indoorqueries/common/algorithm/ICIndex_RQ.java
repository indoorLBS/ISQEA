...
public class ICIndex_RQ {
    ...
    public ArrayList<Integer> iRQ(Point q, double r, RTree<Integer, Box> T) {
        ...
        rangeSearch(q, r, T);
//        System.out.println(R_partitions);
...
//            int parId = IndoorSpace.iPartitions.get(i).getmID();
//            if (!R_partitions.contains(parId)) {
//                IndoorSpace.iPartitions.set(parId, null);
//            }
//        }
//
...
//            }
//        }
        ArrayList<Integer> result = range(q, r, T);
...
//
//        Observable<Entry<Integer, Box>> entries = T.search(Geometries.point(p.getX(), p.getY(),p.getmFloor()));
//
//
//        entries.subscribe(
...
//    }
    ...
    public double pt2objectDist(Point ps, IndoorObject ob, RTree<Integer, Box> T) {
        ...
        for (int i = 0; i < sDoors.size(); i++) {
            ...
            if (tempParId != -1) {
                if (IndoorSpace.iPartitions.get(tempParId) == null) continue;
